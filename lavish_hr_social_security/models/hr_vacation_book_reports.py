...
class hr_vacation_book(models.TransientModel):
    ...
    provisions_id = fields.Many2one('hr.executing.provisions', 'Periodo', required=True)
    final_year = fields.Integer(related='provisions_id.year', string='Año', store=True)
    final_month = fields.Selection(related='provisions_id.month', string='Mes', store=True)
